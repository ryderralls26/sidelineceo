...
export async function createTeamInvite(data: {
  ...
  try {
    ...
    const inviteLink = `${process.env.NEXT_PUBLIC_APP_URL || 'https://sidelinemgmt.space'}/join?token=${invite.token}`;
...
${data.inviterName} has invited you to join ${data.teamName} on SidelineCEO — the free platform for managing flag football teams like a pro.
